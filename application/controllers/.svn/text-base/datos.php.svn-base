...
class Datos_Controller extends Website_Controller {
    ...
    public $ciudad = array('localidad_id!='=>0);
...
//        $this->template->data->messages = '';
    ...
    public function getDatoInfo() {
        if ($this->dato!="") {
            ...
            if ($this->dato->nro!=0) $datoInfo.= ' '.$this->dato->nro;
            if ($this->dato->piso!=0) $datoInfo.= ' '.$this->dato->piso;
            if ($this->dato->depto!="") $datoInfo.= ' '.$this->dato->depto;
